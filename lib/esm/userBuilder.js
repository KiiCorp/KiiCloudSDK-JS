...
import { InvalidPasswordException, InvalidUserIdentifierException, InvalidUsernameException, InvalidEmailException, InvalidPhoneNumberException, } from "./exception";
...
export class KiiUserBuilder {
    ...
    static builderWithLocalPhoneNumber(phoneNumber, country, password, app = KiiApplication.globalApp) {
        ...
        if (!validateCountryCode(country)) {
            throw new InvalidPhoneNumberException("Invalid Country code", undefined);
        }
        if (!validatePassword(password)) {
