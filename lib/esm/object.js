...
export class KiiObject {
     ...
     */
    set(key, value) {
        if (!isNonEmptyString(key) || key.indexOf("_") == 0) {
    ...
    setGeoPoint(key, point) {
        if (key === null || key === "") {
            throw new InvalidArgumentException("Specified key is invalid", this);
        }
        if (point instanceof KiiGeoPoint) {
            this.patch[key] = point.toObject();
        }
        else {
            throw new InvalidArgumentException("Specified latitide or longitude is invalid", this);
        }
    }
    /**
    ...
    getGeoPoint(key) {
        ...
        //   return null;
        // }
        return KiiGeoPoint.fromObject(point);
    }
    /**
    ...
    /** @hidden */
    _getToken() {
        const currUser = KiiUser.getCurrentUser();
    ...
    performCreate(body) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isSuccess(response.status)) {
                ...
                if (objectId == null) {
                    throw error("No objectID response", this);
                }
                this.id = objectId;
    ...
    performUpdate(body, overwrite) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (objectId == null) {
                throw error("empty object ID", this);
            }
            ...
            if (!overwrite) {
                if (isSaved) {
                    if (this.etag.length == 0) {
                        throw error("IllegalState! Call KiiObject#refresh() prior call this method.", this);
                    }
    ...
    /** @hidden */
    static objectWithBucket(bucket, type, app = KiiApplication.globalApp) {
        ...
        for (let key in body) {
            ...
            switch (key) {
                ...
                case "_modified":
                    ...
                    break;
                case "_owner":
                    this.owner = KiiUser.userWithID(value);
                    break;
                case "_dataType":
     ...
     * ```
     */
    static objectWithURI(uri, app = KiiApplication.globalApp) {
        let kiiuri;
        let entity;
        try {
            kiiuri = KiiUri.parse(uri, app);
            entity = kiiuri.toEntity();
        }
        catch (e) {
            throw error(e.message, this);
        }
        if (entity instanceof KiiObject) {
            return entity;
        }
        else {
            throw error("uri is not for object.", undefined);
        }
    }
    /**
    ...
    moveBody(targetObjectUri) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!targetObjectUri) {
                throw new InvalidArgumentException("targetObjectUri is required", this);
            }
            let targetUri;
            let body;
            try {
                targetUri = KiiUri.parse(targetObjectUri, this.app);
