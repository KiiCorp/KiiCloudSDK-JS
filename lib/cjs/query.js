...
exports.KiiClause = exports.KiiQuery = void 0;
const _1 = require(".");
...
class KiiClause {
    ...
    static geoDistance(key, center, radius, putDistanceInto) {
        ...
        const isValidGeoPoint = (point) => {
            ...
            return point instanceof _1.KiiGeoPoint;
        };
        if (!isValidString(key)) {
            throw new exception_1.InvalidArgumentException("Specified key is not a string or is an empty string.", undefined);
        }
        if (!isValidGeoPoint(center)) {
            throw new exception_1.InvalidArgumentException("center is not a reference of KiiGeoPoint.", undefined);
        }
        const pattern = "^[a-zA-Z_][a-zA-Z0-9_]*$";
        if (putDistanceInto && pattern.match(putDistanceInto) !== null) {
            throw new exception_1.InvalidArgumentException("putDistanceInto is invalid.", undefined);
        }
        if (putDistanceInto === "") {
            throw new exception_1.InvalidArgumentException("putDistanceInto is invalid.", undefined);
        }
        if (isNaN(radius) || radius <= 0 || radius > 20000000) {
            throw new exception_1.InvalidArgumentException("radius is invalid.", undefined);
        }
        const expression = new KiiClause();
        let dict = {};
    ...
    static geoBox(key, northEast, southWest) {
        ...
        if (!isValidGeoPoint(northEast) || !isValidGeoPoint(southWest)) {
            throw new exception_1.InvalidArgumentException("northEast or southWest is not a reference of KiiGeoPoint.", undefined);
        }
        const expression = new KiiClause();
        let dict = {};
        dict.type = "geobox";
    ...
    static hasField(key, fieldType) {
        if (!["STRING", "INTEGER", "DECIMAL", "BOOLEAN"].includes(fieldType)) {
            throw new exception_1.InvalidArgumentException("fieldType is invalid.", undefined);
        }
        return KiiClause.create("hasField", key, fieldType);
    }
}
exports.KiiClause = KiiClause;
